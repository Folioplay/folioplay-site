...
export function TeamCreation() {
  const navigate = useNavigate();
  const [wasActiveTab, setWasActiveTab] = useState(localStorage.getItem('CreateTeamTab') || "superstars");
  const [graphCoin, setGraphCoin] = useState("");
  const [open, setOpen] = useState(false);
  const [coins, setCoins] = useState([]);
  const [snackOpen, setSnackOpen] = useState(false);
  ...
  setSnackOpen(false);
    setWasActiveTab(e.toLowerCase());
    localStorage.setItem("CreateTeamTab",e.toLowerCase());
  };
  var prevRoute = state && state.comingFrom ? state.comingFrom : "/activity";
  const LeftComponent = () => {
    return (
        ...
        {CoinsCounterCircle}
          ...
          <div className="coin-classes mb-10">
            <span
              id="superstars-tab"
              className="coinClass coin-class-selected ml-20"
              onClick={(e)=>{changeTabs("superstars")}}
            >
              SuperStars
            </span>
            <span id="superstars-selected-number" className="font-size-12">
             {localStorage.getItem("Selectedsuperstar")}
            </span>
            <span
              id="mooning-tab"
              className="coinClass ml-20"
              onClick={(e)=>{changeTabs("mooning")}}
            >
              Mooning
            </span>
            <span id="mooning-selected-number" className="font-size-12">
            {localStorage.getItem("Selectedmooning")}
            </span>
            <span
              id="rekt-tab"
              className="coinClass ml-20"
              onClick={(e)=>{changeTabs("rekt")}}
            >
              Rekt
            </span>
            <span id="rekt-selected-number" className="font-size-12">
            {localStorage.getItem("SelectedRekt")}
            </span>
          </div>
        </div>
       </>}
